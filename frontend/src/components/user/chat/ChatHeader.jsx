...
export default function ChatHeader({ friend, onBack, typing, onSearch }) {
  ...
  return <header className="chat-header">
    ...
    <div className="chat-header-actions"><button type="button" aria-label="Start voice call" onClick={() => startCall(friend, "VOICE")}><Phone size={18}/></button><button type="button" aria-label="Start video call" onClick={() => startCall(friend, "VIDEO")}><Video size={18}/></button><button type="button" aria-label="Search messages in this conversation" title="Search messages" onClick={onSearch}><Search size={18}/></button><button type="button" aria-label="More options"><MoreVertical size={19}/></button></div>
  </header>;
}