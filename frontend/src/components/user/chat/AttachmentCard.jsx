...
export default function AttachmentCard({ attachment }) {
  ...
  return <div className="chat-attachment-card">
    <span className="chat-attachment-icon"><FileIcon attachment={attachment} size={18} /></span>
    ...
    <span className="chat-attachment-actions">
      <a href={url} target="_blank" rel="noreferrer" aria-label={`Open ${name}`} title="Open"><ExternalLink size={15} /></a>
      <a href={url} download={attachment.attachmentName || true} aria-label={`Download ${name}`} title="Download"><Download size={15} /></a>
    </span>
  </div>;
}
